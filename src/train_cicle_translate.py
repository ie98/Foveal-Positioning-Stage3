import numpy as np
import torch
import dataProcess.cross_cicleMask_translate as DL
...
saveFileName = 'data translate & epochs 200 & lr 0.00001 & lrf 0.00001 & segment & patchSize R & maskSize circleR 25 & randomDeviation 0.25R & batchsize 16'
class trainer():
                ...
                for i, data in tqdm(enumerate(val_LD)):
                    ...
                    for p in range(pred.shape[0]):
                        ...
                        if len(index[0]) == 0:
                            ...
                            pre_x = np.mean(index[1])
                            pre_y = np.mean(index[0])
                        pred_point[p,0] = pre_x
                        pred_point[p,1] = pre_y

                    # 模型成绩测试
                    dist_list = self.eucliDist(pred_point,data[2].detach().cpu().numpy()[:,:])
                    R8_dist = dist_list/R
