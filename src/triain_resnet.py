...
saveFileName = 'crossVal TVT & VAT 0.5 & epochs 400 & segment & patchSize R & maskSize is cicle 21~25 & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def train(self,device):
        ...
        # Dict = torch.load('/home/stu013/mapping/step_3/src/modelParam/MESSIDOR/crossVal TVT & VAT 0.5 & epochs 300 & segment & patchSize R & maskSize 43 & randomDeviation 0.25R & batchsize 16/modelParam_294_6.71866003491306.pth')
        ...
        # for epoch in range(num_epoch,num_epoch+addEpochs):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                # if epoch > 20:
                #     batch_loss = main_loss + 0.5*(epoch/num_epoch) * lds # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
                # else:
            ...
            if epoch >= 20:
                if R_8_per > best_r8 or val_loss < best_valLoss or dist < best_dist :
