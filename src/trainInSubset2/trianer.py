...
saveFileName = 'train in subset2 & crossVal TVT & epochs 500 & lr 0.00001 & lrf 0.0001 & cos num 50 & segment & patchSize R & maskSize circleR 23 & randomDeviation 0.25R & batchsize 16'
class trainer():

    def __init__(self,arg_file=None):
        LOG_FORMAT = "%(asctime)s - %(levelname)s - %(message)s"
        DATE_FORMAT = "%m/%d/%Y %H:%M:%S %p"
        if os.path.exists('./outputFile/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName)) is False:
            os.makedirs('./outputFile/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName))
        if os.path.exists('./outputFile/MESSIDOR_seg_trainInSubset2/{}/my.log'.format(saveFileName)) is False:
            file = open('./outputFile/MESSIDOR_seg_trainInSubset2/{}/my.log'.format(saveFileName), 'w')
            file.close()
        logging.basicConfig(filename='./outputFile/MESSIDOR_seg_trainInSubset2/{}/my.log'.format(saveFileName), level=logging.DEBUG, format=LOG_FORMAT, datefmt=DATE_FORMAT)
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/trainInSubset2/modelParam/MESSIDOR_seg_trainInSubset2/train in subset2 & crossVal TVT & epochs 500 & lr 0.00001 & lrf 0.0001 & cos num 50 & segment & patchSize R & maskSize circleR 23 & randomDeviation 0.25R & batchsize 16/modelParam_211_6.845941762002177.pth')
        ...
        # for epoch in range(num_epoch,num_epoch+addEpochs):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                # main_loss = loss(main_Pred.cuda(device), data[1].cuda(device))
                # if epoch > 0:
                #     batch_loss = main_loss + min(epoch/100,0.5) * lds # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
                # else:
            ...
            with torch.no_grad():
                ...
                if os.path.exists('./outputFile/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName)) is False:
                    os.makedirs('./outputFile/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName))
                dataframe.to_csv('./outputFile/MESSIDOR_seg_trainInSubset2/{}/val_detail_{}.csv'.format(saveFileName,epoch), sep=',')
                if epoch == 0:
                    f = open('./outputFile/MESSIDOR_seg_trainInSubset2/{}/detail.txt'.format(saveFileName),'a+')
            ...
            if epoch >= 20:
                ...
                if R_8_per > best_r8 or val_loss < best_valLoss or dist < best_dist or num_epoch - epoch == 50 or num_epoch - epoch == 40 or num_epoch - epoch == 30 or num_epoch - epoch == 20 or num_epoch - epoch == 10 or num_epoch - epoch == 0:
                    if os.path.exists('./modelParam/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName)) is False:
                        os.makedirs('./modelParam/MESSIDOR_seg_trainInSubset2/{}'.format(saveFileName))
                    ...
                    torch.save(best_model,
                               './modelParam/MESSIDOR_seg_trainInSubset2/{}/modelParam_{}_{}.pth'.format(saveFileName, epoch, dist))
