import numpy as np
import torch
import dataprocess.testDataProcess_subset1 as DL
...
import logging
from utils.computeRrate_R import computerRrate_test
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
basePath = '../../../dataset/MESSIDOR'
saveFileName = 'end test in subset1'
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/trainInSubset2/modelParam/MESSIDOR_seg_trainInSubset2/train in subset2 & crossVal TVT & epochs 500 & lr 0.0001 & lrf 0.0001 & cos num allepoch & segment & patchSize R & maskSize circleR 25 & randomDeviation 0.25R & batchsize 16/modelParam_393_6.882337117698642.pth',map_location={'cuda:1':'cuda:0'})
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(testDataLoader)):
                ...
                for p in range(pred.shape[0]):
                    ...
                    if len(index[0]) == 0 or len(index[0]) < (0.8 * 3.14 * 25 * 25):
                        pre_x = data[2][p,0,0]+int(R/2)
                        pre_y = data[2][p,0,1]+int(R/2)

                    else:
                        print('3333333')
                        pre_x = (np.mean(index[1])+data[2][p,0,0]+data[2][p,0,0]+int(R/2))/2
                        pre_y = (np.mean(index[0])+data[2][p,0,1]+data[2][p,0,1]+int(R/2))/2
                ...
                # score2 = score2 + s2
                if testImgName[i] == '20051214_41949_0100_PP.tif':
                    self.savePreImg(data[3][p], finall_pre, data[2][p], pred_point[p, :],data[1].detach().cpu().numpy()[:,0,:])

            # val_detail = np.concatenate(val_image_names, label_site, pred_site, curr_dist)

            real_r8_count,real_r4_count,real_r2_count,real_r_count,real_rr_count,real_dist = computerRrate.computer(pred_site,'./outputFile/MESSIDOR_test_subset2/{}'.format(saveFileName),293,testImgName.reshape(-1))
            ...
            print('real_dist = {}'.format(real_dist))
            ...
            print('real_r8_per = {}, real_r4_per = {}, real_r2_per = {}, real_r_per = {} , real_rr_per = {}'.format((real_r8_count/568),(real_r4_count/568),(real_r2_count/568),(real_r_count/568),(real_rr_count/568)))
            # 保存用于画图
