import numpy as np
import torch
import dataProcess.crossVal_gaussMask as DL
...
import logging
from vat_gauss import VATLoss
from getDetail.computeRrate import computerRrate_val
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'

saveFileName = 'use gaussMask kl_div & use subset1 & not VAT （pretrained in Vat） & temp crossVal TVT & epochs 500 & lr 0.00001 & lrf 0.001 & cos epoch 50 & segment & patchSize R & gaussMask std 41 & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/modelParam/MESSIDOR/use gaussMask kl_div & use subset1 & VAT 0.2 & temp crossVal TVT & epochs 500 & lr 0.00001 & lrf 0.001 & cos epoch 50 & segment & patchSize R & gaussMask std 41 & randomDeviation 0.25R & batchsize 16/modelParam_190_0.9649122807017544_6.41517968549495.pth')
        ...
        # for epoch in range(num_epoch,num_epoch+addEpochs):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                # batch_loss = F.cross_entropy(F.softmax(main_Pred.cuda(device),dim=1), F.softmax(data[1].cuda(device),dim=1))

                main_Pred = model(data[0].cuda(device))  # 利用 model 得到预测的概率分布，这边实际上是调用模型的 forward 函数
                # main_loss =  F.cross_entropy(F.softmax(main_Pred.cuda(device),dim=1), F.softmax(data[1].cuda(device),dim=1))
                # main_loss = torch.mean((main_Pred.cuda(device) - data[1].cuda(device)) ** 2)
                logp_hat = F.log_softmax(main_Pred.cuda(device), dim=1)
                main_loss = F.kl_div(logp_hat, data[1].cuda(device), reduction='batchmean')
                if epoch > 100000:
                    lds = vat_loss(model, data[0].cuda(device))
                    batch_loss = main_loss + min(epoch/50,0.2) * lds # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
                else:
            ...
            with torch.no_grad():
                ...
                for i, data in tqdm(enumerate(val_LD)):
                    ...
                    vp_arr = val_pred.detach().cpu().numpy()
                    # batch_loss = sum([(x - y) ** 2 for x, y in zip(data[1].cuda(), val_pred)]) / len(train_pred)
                    # batch_loss = F.cross_entropy(F.softmax(val_pred.cuda(device),dim=1), F.softmax(data[1].cuda(device),dim=1))
                    # val_acc += np.sum(np.argmax(val_pred.cpu().data.numpy(), axis=1) == data[1].numpy())
                    # batch_loss = torch.mean((val_pred.cuda(device) - data[1].cuda(device))**2)
                    logp_hat = F.log_softmax(val_pred.cuda(device), dim=1)
                    batch_loss = F.kl_div(logp_hat, data[1].cuda(device), reduction='batchmean')
                    ...
                    for p in range(pred.shape[0]):
                        ...
                        if len(index[0]) == 0:
                            ...
                            #pre_x = (np.sum(index[1] * obj_pre[index[0], index[1]]) / w_sum)+data[3][p,0,0]
                            #pre_y = (np.sum(index[0] * obj_pre[index[0], index[1]]) / w_sum)+data[3][p,0,1]
            ...
            if epoch >= 15:
                ...
                if R_8_per > best_r8 or R_8_per > best_acc  or val_loss < best_valLoss or dist < best_dist or num_epoch - epoch == 50 or num_epoch - epoch == 40 or num_epoch - epoch == 30 or num_epoch - epoch == 20 or num_epoch - epoch == 10 or num_epoch - epoch == 0:
                    ...
                    torch.save(best_model,
                               './modelParam/MESSIDOR/{}/modelParam_{}_{}_{}.pth'.format(saveFileName, epoch,R_8_per, dist))
