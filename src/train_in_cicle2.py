import numpy as np
import torch
import dataProcess.crossVal_circleMask_2 as DL
...
saveFileName = 'train in unetfix & cicleMask inter outer & crossVal TVT & epochs 500 & lr 0.001 & lrf 0.01 & segment & patchSize R & maskSize circleR 30_16 & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def train(self,device):
        ...
        # for epoch in range(num_epoch,num_epoch+addEpochs):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                batch_loss = loss(main_Pred.cuda(device), data[1].cuda(device)) + loss_2(point.cuda(device),data[4].cuda(device))
            ...
            with torch.no_grad():
                ...
                for i, data in tqdm(enumerate(val_LD)):
                    ...
                    batch_loss= loss(val_pred.cuda(device), data[1].cuda(device))+loss_2(point.cuda(device),data[4].cuda(device))
            ...
            if epoch >= 0:
                ...
                if R_8_per_2 > best_r8 or val_loss < best_valLoss or dist_2 < best_dist or num_epoch - epoch == 50 or num_epoch - epoch == 40 or num_epoch - epoch == 30 or num_epoch - epoch == 20 or num_epoch - epoch == 10 or num_epoch - epoch == 0:
                    ...
                    torch.save(best_model,
                               './modelParam/MESSIDOR/{}/modelParam_{}_{}.pth'.format(saveFileName, epoch, dist_1))
