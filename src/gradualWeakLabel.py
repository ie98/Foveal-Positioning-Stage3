...
# 渐进式学习弱标签：
# 首先，我们没有黄斑对应的分割标注，但因为黄斑的形状简单,近似一个以黄斑中央凹为中心的圆，我们只需要知道其长短轴即可。
# 第一个实验中，我们将该圆定义为一个正圆，我们只需要知道圆的半径即可。
# 由于黄斑的边界模糊，并没有出现的边缘信息，那么就难以定义一个统一大小的圆来作为标注。
# 我们在训练过程中，并没有将半径设置为常量，而是将其定义为一个逐步下降的变量。类似于从多尺度空间中选择最佳的尺度。
# 这种方法的好处有以下几点：
# 1. 我们可以先在大的尺度上对黄斑进行粗定位，再缩小我们的尺度，得到一个更加精细的定位。
# 2. 弱标注的尺寸是由样本特征决定的，而不是人为确定。
#
# 实验计划：
# 半径的初始大小为 R/2 ， 每次缩小2个像素。对抗训练权重 初始为 1。
# 每次缩小样本后进行100次迭代，对抗训练权重伴随着半径的缩小而缩小，每次缩小为上一次的 9/10。



os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'

saveFileName = 'temp crossVal TVT & epochs 500 & lr 0.001 & lrf 0.001 & segment & patchSize R & maskSize circleR 23 & randomDeviation 0.25R & batchsize 16'
class trainer():
