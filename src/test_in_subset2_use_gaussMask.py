...
saveFileName = 'train in use gaussMask & temp crossVal TVT & epochs 400 & lr 0.01 & lrf 0.01 & segment & patchSize R & maskSize circleR 23 & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def select_max_region2(self,mask):
    ...
    def train(self,device):
        step2_name = 'threshold_R_rate_kernal_test_inSubset2_resnet18Freeze'
        # 使用tensorBoard
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/modelParam/MESSIDOR/use gaussMask kl_div & use subset1 & no VAT & temp crossVal TVT & epochs 500 & lr 0.00001 & lrf 0.001 & cos epoch 50 & segment & patchSize R & gaussMask std 41 & randomDeviation 0.25R & batchsize 16/modelParam_39_0.9210526315789473_6.46123326959206.pth',map_location={'cuda:1':'cuda:0'})
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(testDataLoader)):
                ...
                val_pred_target = val_pred[:,1,:].detach().cpu().numpy()
                ...
                for p in range(pred.shape[0]):
                    val_pred_target_item = val_pred_target[p,:]
                    cur_pred = pred[p,:]
                    bg_pre = cur_pred[0, :]
                    obj_pre = cur_pred[1, :]
                    ...
                    #    finall_pre = self.select_max_region2(finall_pre.astype(np.uint8))
                    ...
                    if len(index[0]) == 0 or len(index[0]) < int(0.1*3.14*25*25):
                        ...
                        # w_sum_1 = np.sum(obj_pre[index[0], index[1]])
                        # pre_x_1 = (np.sum(index[1] * obj_pre[index[0], index[1]]) / w_sum_1)+data[2][p,0,0]
                        # pre_y_1 = (np.sum(index[0] * obj_pre[index[0], index[1]]) / w_sum_1)+data[2][p,0,1]
                        #
                        # # 横向
                        # w_sum_2 = np.sum(val_pred_target_item[index[0], index[1]])
                        # pre_x_2 = (np.sum(index[1] * val_pred_target_item[index[0], index[1]]) / w_sum_2) + data[2][p, 0, 0]
                        # pre_y_2 = (np.sum(index[0] * val_pred_target_item[index[0], index[1]]) / w_sum_2) + data[2][p, 0, 1]
                        # pred_point[p, 0] = (pre_x_1+pre_x_2)/2
                        # pred_point[p, 1] = (pre_y_1+pre_y_2)/2
                        pred_point[p, 0] = pre_x
                        pred_point[p, 1] = pre_y
            ...
            dataframe.to_csv('./outputFile/MESSIDOR_test_subset2/{}/step3_{}.csv'.format(saveFileName,step2_name), sep=',')
