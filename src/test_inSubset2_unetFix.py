...
saveFileName = 'unetFix & test in subset2 tran in crossVal TVT & epochs 200 & lr 0.0001 & lrf 0.01 & segment & patchSize R & maskSize circleR 25  & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/modelParam/MESSIDOR/train in unetfix pretrained & no freezeWeight & subset1 & crossVal TVT & epochs 200 & lr 0.00001 & lrf 0.001 & segment & patchSize R & maskSize circleR 21 & randomDeviation 0.35R & batchsize 16/modelParam_340_6.822463202310303.pth',map_location={'cuda:0':'cuda:1'})
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(testDataLoader)):
                ...
                pred_2 = point.detach().cpu().numpy()


                # pred[pred>=0.5] = 1
                # pred[pred<0.5] = 0
                pred_point = np.ndarray(shape=[pred_2.shape[0],2])
                for p in range(pred_2.shape[0]):
                    pred_point[p, 0] = pred_2[p, 0] + data[2][p, 0, 0]
                    pred_point[p, 1] = pred_2[p, 1] + data[2][p, 0, 1]
