...
def loadImgs(img_name,label,isVal):
    ...
    x, y = np.mgrid[0:109:110j, 0:109:110j]  # 均值就是标注点 ， 如何根据边缘的位置计算方差
    sigma = (mask_size/2)
    y1 = int(R/2)-random_x
    x1 = int(R/2)-random_y
    ...
    label_mask_target = q
    label_mask_baground =  -q+1

    label_mask = np.concatenate((label_mask_baground.reshape(1,110,110),label_mask_target.reshape(1,110,110)),axis=0)


    # label_mask = label_mask*2
    # img_patch[:,:,2] = label_mask
    ...
    # if os.path.exists('./gaussMask/mask01') is False:
    #     os.makedirs('./gaussMask/mask01')
    # cv2.imwrite('./gaussMask/mask01/{}'.format(img_name), img_patch)

    # patch2 = img[left_y:left_y+110,left_x:left_x+110,2]
    #
    # index2 = np.where(label_mask > 0.4)
    # patch2[index2[0],index2[1]] = label_mask[index2[0],index2[1]]
    # img[left_y:left_y + 110, left_x:left_x + 110, 2] = patch2
    #
    # if os.path.exists('./gaussMask/mask02') is False:
    #     os.makedirs('./gaussMask/mask02')
    # cv2.imwrite('./gaussMask/mask02/{}'.format(img_name), img)
...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return X_img, Y_label.astype(np.float32),new_labels,leftUp_point,label_in_Patch
