...
def loadImgs(img_name,label,isVal):
    ...
    left_x = patch_center_x - int(patch_size/2)
    left_y = patch_center_y - int(patch_size/2)
    leftUp_point = np.ndarray(shape=[1, 2])
    leftUp_point[0,0] = left_x
    leftUp_point[0,1] = left_y

    # 截取输入样本图像
    img_patch = img[patch_center_y-int(patch_size/2):patch_center_y+patch_size-int(patch_size/2),patch_center_x-int(patch_size/2):patch_center_x+patch_size-int(patch_size/2),:]
    ...
    center_row, center_col = int(patch_size/2)-random_y, int(patch_size/2)-random_x
    ...
    label_in_patch[0,0] = int(patch_size/2) -  random_x
    label_in_patch[0,1] = int(patch_size/2) -  random_y
