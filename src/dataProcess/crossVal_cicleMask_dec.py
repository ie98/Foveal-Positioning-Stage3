...
def loadImgs(img_name,label,maskSize):
    R = 109
    patch_size = R+1
    # if isVal:
    #     mask_size = 23
...
class MyDataSet(Dataset):
    ...
    def __init__(self, x_img_name, y_label,maskSize, transforms=None):
    ...
    def __getitem__(self, item):
        X_img, Y_label,new_labels,leftUp_point,label_in_Patch = loadImgs(self.x_img_name[item], self.y_label[item].reshape(1,2),self.maskSize)
...
def getDataLoader(batchSize,maskSize):
    print('maskSize = {}'.format(maskSize))
    labelFileName = 'data.xls'
    ...
    train_set = MyDataSet(train_imgNames,train_labels,maskSize,input_transform)
    val_set = MyDataSet(val_imgName,val_labels,maskSize,input_transform)
