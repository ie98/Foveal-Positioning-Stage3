...
def loadImgs(img_name,label):
    ...
    return img_patch,new_labels,leftUp_point
...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, item):
        X_img,new_labels,leftUp_point = loadImgs(self.x_img_name[item], self.y_label[item].reshape(1,2))
        if self.transforms is not None:
            X_img = self.transforms(X_img)
        return X_img,new_labels,leftUp_point
