...
def translate(img,label):
    hor_flip = random.uniform(0,1)
    ver_flip = random.uniform(0,1)
    rotate = random.randint(0,4)

    if hor_flip >= 0.5:
        img = img.transpose(Image.FLIP_LEFT_RIGHT)
        # mask.transpose(Image.FLIP_LEFT_RIGHT)
        label[0,0] = 109 - label[0,0]
    if ver_flip >= 0.5:
        img = img.transpose(Image.FLIP_TOP_BOTTOM)
        # mask.transpose(Image.FLIP_TOP_BOTTOM)
        label[0,1] = 109 - label[0,1]
...
def loadImgs(img_name,label,isVal):
    ...
    label_in_patch[0, 0] = int(int(R / 2) - random_x)
    label_in_patch[0, 1] = int(int(R / 2) - random_y)

    # label_point_value = img_patch[int(label_in_patch[0,1]),int(label_in_patch[0,0]),:]
    # print('liv_1:{}'.format(label_point_value))
    # temp_patch1 = img_patch.copy()
    # temp_patch1[int(label_in_patch[0, 1]) - 3:int(label_in_patch[0, 1]) + 3,
    # int(label_in_patch[0, 0]) - 3:int(label_in_patch[0, 0]) + 3, :] = (0, 0, 255)
    # cv2.imwrite('./origin_{}.png'.format(img_name),temp_patch1)

    patch_Image = Image.fromarray(img_patch)

    img_patch_2, label_in_patch_2 = translate(patch_Image, label_in_patch)

    # label_point_value = img_patch_2[int(label_in_patch_2[0,1]),int(label_in_patch_2[0,0]),:]
    # print('liv_2:{}'.format(label_point_value))
    # temp_patch2 = img_patch_2.copy()
    # temp_patch2[int(label_in_patch_2[0,1])-3:int(label_in_patch_2[0,1])+3,int(label_in_patch_2[0,0])-3:int(label_in_patch_2[0,0])+3,:] = (0,0,255)
    # cv2.imwrite('./translate_{}.png'.format(img_name),temp_patch2)
    ...
    center_row, center_col = label_in_patch_2[0, 1], label_in_patch_2[0, 0]
    ...
    return img_patch_2,label_mask,label_in_patch_2
...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, item):
        X_img, Y_label,label_in_Patch = loadImgs(self.x_img_name[item], self.y_label[item].reshape(1,2),self.isVal)
        if self.transforms is not None:
            X_img = self.transforms(X_img)
        return X_img, Y_label.astype(np.int),label_in_Patch.reshape(2).astype(np.float32)
