...
def loadImgs(img_name,label,step2Pre):
    ...
    random_x = random.randint(int(-0.1 * R), int(0.1 * R))
    random_y = random.randint(int(-0.1 * R), int(0.1 * R))

    patch_center_x = new_s2_x + random_x
    patch_center_y = new_s2_y + random_y
    ...
    return img_patch,new_labels,leftUp_point,img_name,random_x,random_y
...
class TestDataSet(Dataset):
    ...
    def __getitem__(self, item):
        X_img,new_labels,leftUp_point,img_name,random_x,random_y = loadImgs(self.x_img_name[item], self.y_label[item].reshape(1,2),self.step2Pre[item].reshape(1,2))
        if self.transforms is not None:
            X_img = self.transforms(X_img)
        return X_img,new_labels,leftUp_point,img_name,random_x,random_y
...
def getDataLoader(batchSize):
    ...
    step2Pred = np.array(pd.read_csv('./step2Pred/threshold_R_rate_kernal_test_inSubset2_resnet18Freeze.csv'))
