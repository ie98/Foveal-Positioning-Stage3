import numpy as np
import torch
import dataProcess.testDataProcessTVT as DL
# from network.mainNet.mainNet import mainNet
from net_gaussMask.unet_model import UNet
...
import logging
from utils import thresholdSeg
from utils.computeRrate import computerRrate_test
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
basePath = '../../dataset/MESSIDOR'
saveFileName = 'randomSample & test in subset1'
class trainer():
    ...
    def savePreImg(self,imageName,mask,left_index,pred_point,label):
        ...
        if os.path.exists('./outputFile/MESSIDOR_test_subset1/{}/preMask'.format(saveFileName)) is False:
            os.makedirs('./outputFile/MESSIDOR_test_subset1/{}/preMask'.format(saveFileName))
        cv2.imwrite('./outputFile/MESSIDOR_test_subset1/{}/preMask/{}.png'.format(saveFileName,imageName),img)


    def train(self,device):
        step2_name = 'randomSample_subset2'
        # 使用tensorBoard
        print('Strat Tensorboard with "tensorboard -- logdir=runs",view at http://locahost:6006/')
        if os.path.exists('./outputFile/MESSIDOR_test_subset1/{}'.format(saveFileName)) is False:
            os.makedirs('./outputFile/MESSIDOR_test_subset1/{}'.format(saveFileName))
        tb_writer = SummaryWriter()
        device = device

        model = UNet(n_channels=3,n_classes=1).cuda(device)
        Dict = torch.load('/home/stu013/mapping/step_3/src/trainInRandomSample/trainInSubset2_gaussMask/modelParam/MESSIDOR/randomSample & crossVal TVT & no VAT & epochs 400 & lr 0.0001 & lrf 0.01 & segment & patchSize R & gauss maskSize 36 & randomDeviation 0.25R & batchsize 16/modelParam_135_6.345109596912837.pth',map_location={'cuda:0':'cuda:1'})
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(enumerate(testDataLoader)):
                ...
                for p in range(pred.shape[0]):
                    ...
                    # pre_x, pre_y = thresholdSeg.thresholdSegment(pred[p, 0, :, :])
                    # pre_x = pre_x + data[2][p, 0, 0]
                    # pre_y = pre_y + data[2][p, 0, 1]
                    maxValue = np.max(pred[p, 0, :, :])
                    # if maxValue < (255/2):
                    #     pre_x = int(109/2) + data[2][p, 0, 0]
                    #     pre_y = int(109/2) + data[2][p, 0, 1]
                    #     stage2Count = stage2Count + 1
                    # else:
                    index = np.where(pred[p, 0, :, :] >= (0.5 * maxValue))
                    w_sum = np.sum(pred[p, 0, index[0], index[1]])
                    pre_x = (np.sum(index[1] * pred[p, 0, index[0], index[1]]) / w_sum)+data[2][p,0,0]
                    pre_y = (np.sum(index[0] * pred[p, 0, index[0], index[1]]) / w_sum)+data[2][p,0,1]
                    ...
                    # print(len(index[0]))
                    # if len(index[0]) == 0 or len(index[0]) < int(0.1*3.14*maskSize*maskSize):
                    #     pre_x = data[2][p,0,0]+int(109/2)
                    #     pre_y = data[2][p,0,1]+int(109/2)
                    # else:
                    #     pre_x = np.mean(index[1])+data[2][p,0,0]
                    #     pre_y = np.mean(index[0])+data[2][p,0,1]
            ...
            real_r8_count,real_r4_count,real_r2_count,real_r_count = computerRrate.computer(pred_site,'./outputFile/MESSIDOR_test_subset2/{}'.format(saveFileName),'R_step3_{}'.format(step2_name),testImgName.reshape(-1))
            ...
            dataframe.to_csv('./outputFile/MESSIDOR_test_subset1/{}/step3_{}.csv'.format(saveFileName,step2_name), sep=',')
