import numpy as np
import torch
import dataProcess.dataProcessTVT as DL
...
saveFileName = 'randomSample & crossVal TVT & no VAT & epochs 400 & lr 0.0001 & lrf 0.01 & segment & patchSize R & gauss maskSize 36 & randomDeviation 0.25R & batchsize 16'
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/step_3/src/trainInRandomSample/trainInSubset2_gaussMask/modelParam/MESSIDOR/randomSample & train in subset2 & crossVal TVT & VAT 0.5 & epochs 400 & lr 0.0001 & lrf 0.01 & segment & patchSize R & gauss maskSize 41 & randomDeviation 0.25R & batchsize 16/modelParam_128_7.212645706090109.pth')
        ...
        # Scheduler 学习率下降曲线
        lf = lambda x: ((1 + math.cos(x * math.pi / 30)) / 2) * (1 - lrf) + lrf  # cosine
        ...
        # for epoch in range(num_epoch,num_epoch+addEpochs):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                batch_loss = torch.mean((main_Pred.cuda(device)-data[1].cuda(device))**2)
                # if epoch >= 5:
            ...
            with torch.no_grad():
                ...
                for i, data in tqdm(enumerate(val_LD)):
                    ...
                    for p in range(pred.shape[0]):
                        ...
                        #     pre_x = pre_x + data[3][p, 0, 0]
                        #     pre_y = pre_y + data[3][p, 0, 1]
                        # else:
            ...
            if epoch >= 20:
                ...
                if R_8_per > best_r8 or val_loss < best_valLoss or dist < best_dist or num_epoch - epoch == 50 or num_epoch - epoch == 40 or num_epoch - epoch == 30 or num_epoch - epoch == 20 or num_epoch - epoch == 10 or num_epoch - epoch == 0:
                    ...
                    torch.save(best_model,
                               './modelParam/MESSIDOR/{}/modelParam_{}_{}.pth'.format(saveFileName, epoch, dist))
        dataframe = pd.DataFrame({'val_loss':val_loss_list})
        dataframe.to_csv('./outputFile/MESSIDOR_seg/{}/val_loss.csv'.format(saveFileName))
