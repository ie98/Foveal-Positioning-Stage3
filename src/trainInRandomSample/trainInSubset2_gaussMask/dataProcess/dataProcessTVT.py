...
def loadImgs(img_name,label,isVal):
    ...
    label_mask = q.reshape(1, 110, 110)*255
...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return X_img, Y_label.astype(np.float32),new_labels,leftUp_point,label_in_Patch.reshape(2).astype(np.float32)
...
def getDataLoader(batchSize):
    ...
    randomSampleFile = pd.read_csv('./dataProcess/randomSample/randomSample.csv')
    ...
    sub_2_train = sub_set_2[[i for i in range(568) if i % 5 != 0]]
    sub_2_val = sub_set_2[[i for i in range(568) if i % 5 == 0]]
    #random_list = random.sample(range(0, len(sub_set_1)), len(sub_set_1))
    #np_list = np.array(random_list)
    train_imgNames = imgNames[sub_2_train]
    train_labels = labels[sub_2_train].astype(np.float32)
    #train_imgNames = train_imgNames[np_list]
    #train_labels = train_labels[np_list]
    val_imgName = imgNames[sub_2_val]
    val_labels = labels[sub_2_val].astype(np.float32)

    test_imgNames = imgNames[sub_set_1]
    test_labels = labels[sub_set_1]
